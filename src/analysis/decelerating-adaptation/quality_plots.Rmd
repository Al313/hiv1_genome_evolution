...
quals_gathered_iii <- quals_gathered %>% filter(virus_line_no %in% 13:16) %>%
        ...
        mutate(virus_line_no = factor(virus_line_no, levels = names(line_col_palette)))
...
depth_summary <- ggplot(data=quals_gathered_iii, aes(x = amplicon, y = average_depth,fill=virus_line_no))+
    ...
    labs(title = "Sequencing Depth", x = "Amplicons", y = "Average sequencing depth (log10)", fill = "Population") +
    scale_x_discrete(labels = c("5'UTR", "Amp_A","Amp_B","Amp_C","Amp_D","Amp_E", "3'UTR")) +
    scale_y_continuous(limit = c(-0.5,5), breaks = c(1,2,log10(200),3,4,5), labels = c("1", "2", "~2.3", "3", "4", "5")) +
    geom_hline(yintercept = mean(quals_gathered_iii$average_depth), linetype="solid") +
    geom_hline(yintercept = log10(200), linetype="dashed") +
    custom_plot_theme(application = "publication")


# save figure
custom_plot_save(depth_summary, "Sequencing_depth_box_plot.pdf", height = 14, width = 14)
