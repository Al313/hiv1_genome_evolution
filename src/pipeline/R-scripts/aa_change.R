...
ann <- str_split(ann_var$ANN, pattern = ",")
names(ann) <- paste0(ann_var$ID,".")

# make a boolean vector of the annotations that we want to include
...
# unlist the annotations to store them in a df (the added dot to the names is handy here)
ann_of_interest <- unlist(ann_of_interest, use.names = T)

# get the annotation fields that are of interest
