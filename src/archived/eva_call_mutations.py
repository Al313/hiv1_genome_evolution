...
#    print(len(fractions[1]))
    ...
    for new, loc in zip(muts[0],muts[1]):
        ...
        if coverage_here>min_coverage:
            muts_to_return.append(('M',loc,bases[new],fraction,n_changed,coverage_here))
            ...
            if 'H' in read.cigarstring or 'S' in read.cigarstring:
                ...
                if 'SA' in tags.keys():
                    ...
                    if int(fields[4])>30: #if mapping quality of the chimera is high enough
                        ...
                        chimera_start_ref = int(fields[1])
...
def find_indels(samfile,coverage,contig_name,muts=None,n=None,start=0,stop=None,
    ...
    for i in muts:
        pos = i[1]
        coverage_here = max(coverage[pos-10:pos+10])
        if coverage_here>min_coverage:
            muttype = i[0]
            mutation = i[2]
            counts = muts[i]
            ...
            if fraction > 1:
                fraction = 1
            muts_to_return.append((muttype,pos,mutation,fraction,counts,coverage_here))
    return muts_to_return
